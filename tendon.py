...
class TendonAnalysis:
    ...
    def optimize(self, ksteps, ijackings, x0, xb):
        ...
        result = optimize.minimize(objfn, my.util.x2r(x0, xb), method='L-BFGS-B', bounds=my.util.make_rb(xb), options={'gtol': 1e-7}, disp=True)
...
class TendonAnalysisTemp:
    ...
    def optimize(self, ksteps, ijackings, x0, xb):
        ...
        # result = optimize.minimize(objfn, my.util.x2r(x0, xb), method='L-BFGS-B', bounds=my.util.make_rb(xb), options={'gtol': 1e-7}, disp=True)
        # result = optimize.basinhopping(objfn, my.util.x2r(x0, xb), minimizer_kwargs={'method': 'L-BFGS-B', 'disp'=True})
        result = optimize.differential_evolution(objfn, my.util.make_rb(xb), disp=True)
        self.mu[:], self.kp[:], self.slip[:], self.Ec = my.util.r2x(result.x, xb)
    ...
    def _solve_equilibriums_at_section(self, ijacking, jsection, Ec, stage):
        ...
        section_strain = np.linalg.solve(A, b)  # following codes: Temporary codes <-----------------------------------------------------------------------------------------------
        # A00 = A[0, 0]
        # b0 = b[0]
        # x0 = b0 / A00
        # A = np.zeros((3, 3))
        # b = np.zeros((3))
        # section_strain = np.zeros(3)
        # A[0, 0] = A00
        # b[0] = b0
        # section_strain[0] = x0
